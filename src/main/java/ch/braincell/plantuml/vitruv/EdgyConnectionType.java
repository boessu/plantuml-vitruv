...
/**
 * Enum representing the different connection types in the Edgy Enterprise
 * Design Foundation. These types indicate the style of a connection when
...
public enum EdgyConnectionType implements ConnectionType {
	/**
	 * the unidirectional link relationship corresponds to a not further specified
	...
	FLOW("-?->"),
	/**
	 * any hierarchical representation of the same element is a tree relationship,
	 * which corresponds to an aggregation relationship.
	 */
	TREE("-?-*");
	...
	EdgyConnectionType(String plant) {
		...
		defaultStyle = new ConnectionStyle(this);
	}
